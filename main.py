...
# ─── App ──────────────────────────────────────────────────────────────────────
app = FastAPI(title="🏛️ Persepolis Gateway v11", docs_url=None, redoc_url=None)
...
async def send_bot_message(chat_id: str, text: str):
...
{'🔹 /status - وضعیت پنل\n🔹 /users - لیست کاربران\n🔹 /stats - آمار پنل' if is_admin else '🔹 /help - راهنما'}
...
async def start_bot():
    ...
    try:
        ...
        BOT_APP.add_handler(CommandHandler("users", bot_users))
...
async def load_state():
    ...
    try:
        ...
        if DATA_FILE.exists():
            ...
            if SETTINGS.get("bot_token") and SETTINGS.get("bot_admin_chat_id") and SETTINGS.get("bot_enabled"):
...
async def startup():
    ...
    log_activity("system", "🏛️ Persepolis Gateway v11 راه‌اندازی شد", "ok")
    logger.info(f"🏛️ Persepolis Gateway v11 started on port {CONFIG['port']}")
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=CONFIG["port"], log_level="info", workers=1)import asyncio
import json
import os
import hashlib
import secrets
