# pages.py - نسخه پایدار برای ریل‌وی
...
<script>
const translations={fa:{welcome:"خوش آمدید",sub:"وارد حساب کاربری خود شوید",username:"نام کاربری یا ایمیل",password:"رمز عبور",remember:"مرا به خاطر بسپار",login:"ورود",or:"یا",connect:"اتصال با یک کلیک",signup:"حساب کاربری ندارید؟",signup_link:"ثبت نام",secure:"امن",secure_d:"حریم خصوصی شما",fast:"سریع",fast_d:"سرعت برق آسا",global:"جهانی",global_d:"سرورهای جهانی",anon:"ناشناس",anon_d:"خصوصی بمانید",info_title:"🪐 پنل عقاب",info_sub:"سریع‌ترین و امن‌ترین اتصال"},en:{welcome:"Welcome Back",sub:"Login to your VPN account",username:"Username or Email",password:"Password",remember:"Remember me",login:"Login",or:"OR",connect:"Connect with One Click",signup:"Don't have an account?",signup_link:"Sign up",secure:"Secure",secure_d:"Your Privacy",fast:"Fast",fast_d:"Lightning Speed",global:"Global",global_d:"Worldwide Servers",anon:"Anonymous",anon_d:"Stay Private",info_title:"🪐 Eagle Panel",info_sub:"Fastest & Most Secure Connection"}};
let currentLang=localStorage.getItem('eagle-lang')||'fa';
function setLang(lang){currentLang=lang;localStorage.setItem('eagle-lang',lang);document.querySelectorAll('.lang-toggle button').forEach(b=>b.classList.toggle('active',b.textContent.includes(lang==='fa'?'فارسی':'English')));updateTexts()}
...
function setTheme(theme){currentTheme=theme;localStorage.setItem('eagle-theme',theme);if(theme==='light'){document.body.classList.add('light-theme');document.getElementById('current-theme-label').textContent='روشن';document.getElementById('theme-dark-btn').className='btn btn-o';document.getElementById('theme-light-btn').className='btn btn-pur'}else{document.body.classList.remove('light-theme');document.getElementById('current-theme-label').textContent='تاریک';document.getElementById('theme-dark-btn').className='btn btn-pur';document.getElementById('theme-light-btn').className='btn btn-o'}fetch('/api/settings/theme',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({theme:theme})}).catch(()=>{})}
async function loadThemeFromServer(){try{const r=await fetch('/api/settings');const data=await r.json();if(data.theme){currentTheme=data.theme;localStorage.setItem('eagle-theme',data.theme);setTheme(data.theme)}else{setTheme(currentTheme)}}catch(e){setTheme(currentTheme)}}
...
# ===== تابع ساب‌لینک ساده و حرفه‌ای (بدون خطا) =====
def get_sub_page_html(uuid: str, link: dict) -> str:
    """صفحه ساب‌لینک با طراحی حرفه‌ای و تایمر زنده"""
    ...
    # محاسبه زمان باقیمانده برای تایمر
    remaining_seconds = 0
    ...
    # نام انگشت‌نگاری
    fp_names = {'chrome':'Chrome','firefox':'Firefox','safari':'Safari','edge':'Edge','ios':'iOS','android':'Android','safari_ios':'Safari iOS','random':'Random','none':'None'}
    fp_name = fp_names.get(fingerprint, fingerprint)
    fp_emoji = {'chrome':'🌐','firefox':'🦊','safari':'🧭','edge':'🌊','ios':'📱','android':'🤖','safari_ios':'🍏','random':'🎲','none':'🚫'}.get(fingerprint, '🔑')
    
    # ساخت HTML
    html = '<!DOCTYPE html>\n'
    html += '<html lang="fa" dir="rtl" data-theme="dark">\n'
    ...
    html += '<title>🪐 ' + label + ' · پنل عقاب</title>\n'
    html += '<link rel="preconnect" href="https://fonts.googleapis.com">\n'
    html += '<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">\n'
    html += '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">\n'
    html += '<style>\n'
    html += '*{margin:0;padding:0;box-sizing:border-box}\n'
    html += ':root{--bg:#0a0a1a;--bg2:#12122a;--bg3:#1a1a3a;--card:rgba(10,10,30,0.8);--card-border:rgba(100,80,255,0.06);--card-hover:rgba(100,80,255,0.1);--text:#F0EEFF;--text2:#8888BB;--text3:#555577;--accent:#7C6BFF;--accent2:#A78BFA;--accent3:#5B4BD9;--green:#10B981;--green-bg:rgba(16,185,129,0.08);--green-text:#34D399;--red:#EF4444;--red-bg:rgba(239,68,68,0.08);--red-text:#F87171;--amber:#F59E0B;--amber-bg:rgba(245,158,11,0.08);--amber-text:#FCD34D;--shadow:0 25px 80px rgba(0,0,0,0.6),0 0 120px rgba(100,80,255,0.02);--glow:0 0 80px rgba(100,80,255,0.03);--transition:all 0.4s cubic-bezier(0.34,1.56,0.64,1);--radius:14px}\n'
    html += '[data-theme="light"]{--bg:#f0f0f8;--bg2:#e8e8f0;--bg3:#dddde8;--card:rgba(255,255,255,0.85);--card-border:rgba(100,80,255,0.12);--card-hover:rgba(100,80,255,0.15);--text:#1a1a2e;--text2:#4a4a6a;--text3:#7a7a9a;--shadow:0 25px 80px rgba(0,0,0,0.08),0 0 120px rgba(100,80,255,0.02);--glow:0 0 80px rgba(100,80,255,0.02)}\n'
    html += 'body{font-family:"Vazirmatn",sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:16px;background:var(--bg);color:var(--text);transition:var(--transition);position:relative;overflow-x:hidden}\n'
    html += '.stars-container{position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden}\n'
    html += '.star{position:absolute;border-radius:50%;background:var(--text);animation:twinkle 4s ease-in-out infinite}\n'
    html += '@keyframes twinkle{0%,100%{opacity:0.1}50%{opacity:0.5}}\n'
    html += '@keyframes float{0%,100%{transform:translate(0,0) scale(1)}50%{transform:translate(20px,-20px) scale(1.03)}}\n'
    html += '.glow-orb{position:fixed;border-radius:50%;filter:blur(120px);z-index:0;pointer-events:none;animation:float 8s ease-in-out infinite}\n'
    html += '.glow-orb1{width:400px;height:400px;background:rgba(100,80,255,0.04);top:-100px;right:-50px}\n'
    html += '.glow-orb2{width:300px;height:300px;background:rgba(167,139,250,0.03);bottom:-50px;left:-30px;animation-delay:3s}\n'
    html += '.card{position:relative;z-index:10;background:var(--card);backdrop-filter:blur(30px);border:1px solid var(--card-border);border-radius:var(--radius);padding:22px 20px 18px;max-width:420px;width:100%;box-shadow:var(--shadow);animation:cardIn 0.6s ease;transition:var(--transition)}\n'
    html += '@keyframes cardIn{from{opacity:0;transform:translateY(30px) scale(0.96)}to{opacity:1;transform:translateY(0) scale(1)}}\n'
    html += '.card-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid var(--card-border)}\n'
    html += '.brand{display:flex;align-items:center;gap:8px}\n'
    html += '.brand-icon{width:32px;height:32px;border-radius:8px;background:linear-gradient(135deg,#7C6BFF,#5B4BD9);display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 0 30px rgba(100,80,255,0.15)}\n'
    html += '.brand-text{font-size:11px;font-weight:700;background:linear-gradient(135deg,#A78BFA,#7C6BFF);-webkit-background-clip:text;-webkit-text-fill-color:transparent}\n'
    html += '.brand-sub{font-size:6px;color:var(--text3);-webkit-text-fill-color:var(--text3)}\n'
    html += '.theme-toggle{background:rgba(100,80,255,0.05);border:1px solid var(--card-border);border-radius:6px;padding:4px 8px;font-size:12px;cursor:pointer;color:var(--text2);transition:var(--transition);font-family:inherit}\n'
    html += '.theme-toggle:hover{background:rgba(100,80,255,0.1);transform:scale(1.05)}\n'
    html += '.user-row{display:flex;align-items:center;justify-content:space-between;margin-bottom:2px}\n'
    html += '.user-name{font-size:18px;font-weight:800;color:var(--text);display:flex;align-items:center;gap:6px}\n'
    html += '.status-badge{display:inline-flex;align-items:center;gap:4px;padding:2px 10px;border-radius:12px;font-size:9px;font-weight:700}\n'
    html += '.status-badge.active{background:var(--green-bg);color:var(--green-text);border:1px solid rgba(16,185,129,0.1)}\n'
    html += '.status-badge.inactive{background:var(--red-bg);color:var(--red-text);border:1px solid rgba(239,68,68,0.1)}\n'
    html += '.uuid-box{background:rgba(100,80,255,0.03);border:1px solid var(--card-border);border-radius:6px;padding:4px 8px;font-size:8px;font-family:monospace;color:var(--text3);word-break:break-all;cursor:pointer;transition:var(--transition);margin:4px 0 8px;text-align:center}\n'
    html += '.uuid-box:hover{background:rgba(100,80,255,0.06);transform:scale(1.01)}\n'
    html += '.conns-section{background:rgba(100,80,255,0.02);border:1px solid var(--card-border);border-radius:8px;padding:6px 10px;margin:6px 0}\n'
    html += '.conns-header{font-size:9px;font-weight:700;color:var(--text2);display:flex;align-items:center;gap:4px;margin-bottom:3px}\n'
    html += '.conns-dot{display:inline-block;width:5px;height:5px;border-radius:50%;background:var(--green-text);animation:pulse 1.5s infinite}\n'
    html += '@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.3;transform:scale(0.6)}}\n'
    html += '.conns-list{display:flex;flex-wrap:wrap;gap:3px}\n'
    html += '.conn-chip{font-family:monospace;font-size:7px;background:rgba(100,80,255,0.04);border:1px solid var(--card-border);padding:1px 6px;border-radius:3px;color:var(--text2)}\n'
    html += '.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:4px;margin:6px 0}\n'
    html += '.info-item{background:rgba(100,80,255,0.02);border:1px solid var(--card-border);border-radius:6px;padding:5px 8px;display:flex;justify-content:space-between;align-items:center;transition:var(--transition)}\n'
    html += '.info-item:hover{background:rgba(100,80,255,0.04)}\n'
    html += '.info-label{font-size:7px;color:var(--text3);display:flex;align-items:center;gap:2px;font-weight:600}\n'
    html += '.info-label i{font-size:8px;color:var(--accent)}\n'
    html += '.info-value{font-size:10px;font-weight:700;color:var(--text)}\n'
    html += '.info-value.small{font-size:8px}\n'
    html += '.timer-section{background:linear-gradient(135deg,rgba(100,80,255,0.04),rgba(167,139,250,0.02));border:1px solid var(--card-border);border-radius:8px;padding:6px 10px;margin:6px 0;text-align:center}\n'
    html += '.timer-label{font-size:7px;color:var(--text3);font-weight:600;text-transform:uppercase;letter-spacing:0.05em}\n'
    html += '.timer-display{font-family:monospace;font-size:18px;font-weight:800;color:var(--accent2);letter-spacing:2px;background:rgba(0,0,0,0.2);padding:4px 10px;border-radius:6px;display:inline-block;margin-top:2px}\n'
    html += '.timer-display.expired{color:var(--red-text)}\n'
    html += '.progress-section{margin:6px 0}\n'
    html += '.progress-bar{height:4px;border-radius:4px;background:rgba(100,80,255,0.05);overflow:hidden}\n'
    html += '.progress-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,#7C6BFF,#5B4BD9,#A78BFA);width:0%;transition:width 1.2s ease}\n'
    html += '.progress-text{display:flex;justify-content:space-between;font-size:7px;color:var(--text3);margin-top:2px}\n'
    html += '.progress-text .pct{font-weight:700;color:var(--text2)}\n'
    html += '.vless-section{background:rgba(100,80,255,0.02);border:1px solid var(--card-border);border-radius:8px;padding:6px 8px;margin:6px 0}\n'
    html += '.vless-label{font-size:7px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.04em;display:flex;align-items:center;gap:4px;margin-bottom:3px}\n'
    html += '.vless-label i{color:var(--accent);font-size:9px}\n'
    html += '.vless-link{font-family:monospace;font-size:7px;color:var(--accent2);word-break:break-all;line-height:1.6;background:rgba(0,0,0,0.15);padding:4px 6px;border-radius:4px;border:1px solid var(--card-border)}\n'
    html += '.vless-ports{display:flex;flex-wrap:wrap;gap:3px;margin-top:4px}\n'
    html += '.vless-port{font-size:7px;background:rgba(100,80,255,0.04);border:1px solid var(--card-border);padding:1px 6px;border-radius:3px;color:var(--text3);font-family:monospace}\n'
    html += '.actions{display:flex;gap:4px;margin-top:8px;flex-wrap:wrap}\n'
    html += '.btn{font-family:inherit;font-size:9px;font-weight:600;border-radius:6px;padding:5px 10px;cursor:pointer;display:inline-flex;align-items:center;gap:3px;border:none;transition:var(--transition);white-space:nowrap;flex:1;justify-content:center}\n'
    html += '.btn i{font-size:10px}\n'
    html += '.btn-primary{background:linear-gradient(135deg,#7C6BFF,#5B4BD9);color:#fff;box-shadow:0 3px 15px rgba(100,80,255,0.15)}\n'
    html += '.btn-primary:hover{transform:translateY(-2px);box-shadow:0 6px 25px rgba(100,80,255,0.25)}\n'
    html += '.btn-secondary{background:rgba(100,80,255,0.03);border:1px solid var(--card-border);color:var(--text2)}\n'
    html += '.btn-secondary:hover{background:rgba(100,80,255,0.06);color:var(--text);transform:translateY(-2px)}\n'
    html += '.btn-success{background:var(--green-bg);border:1px solid rgba(16,185,129,0.08);color:var(--green-text)}\n'
    html += '.btn-success:hover{background:rgba(16,185,129,0.12);transform:translateY(-2px)}\n'
    html += '.footer{margin-top:10px;padding-top:8px;border-top:1px solid var(--card-border);text-align:center;font-size:6px;color:var(--text3)}\n'
    html += '.footer .eagle{color:var(--accent);font-weight:700}\n'
    html += '.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%) translateY(40px);background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-border);color:var(--text);border-radius:8px;padding:6px 14px;font-size:9px;opacity:0;transition:var(--transition);z-index:999;pointer-events:none;box-shadow:var(--shadow);display:flex;align-items:center;gap:4px}\n'
    html += '.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}\n'
    html += '.toast.ok{border-color:rgba(16,185,129,0.15);color:var(--green-text)}\n'
    html += '@media(max-width:400px){.card{padding:14px 12px 12px}.user-name{font-size:15px}.info-grid{grid-template-columns:1fr 1fr}.info-item{padding:3px 6px}.info-value{font-size:8px}.btn{font-size:8px;padding:4px 8px}.timer-display{font-size:14px}}\n'
    html += '</style>\n'
    html += '</head>\n'
    html += '<body>\n'
    ...
    stars = [{'w':2,'h':2,'t':'8%','l':'6%','d':'0s'},{'w':3,'h':3,'t':'25%','l':'35%','d':'1.5s'},{'w':1,'h':1,'t':'45%','l':'65%','d':'0.8s'},{'w':2,'h':2,'t':'65%','l':'15%','d':'2.2s'},{'w':3,'h':3,'t':'80%','l':'75%','d':'0.5s'},{'w':1,'h':1,'t':'35%','l':'85%','d':'1.8s'},{'w':2,'h':2,'t':'55%','l':'45%','d':'2.5s'}]
    for s in stars:
        html += '<div class="star" style="width:'+str(s['w'])+'px;height:'+str(s['h'])+'px;top:'+s['t']+';left:'+s['l']+';animation-delay:'+s['d']+'"></div>\n'
    html += '</div>\n'
    html += '<div class="glow-orb glow-orb1"></div>\n'
    html += '<div class="glow-orb glow-orb2"></div>\n'
    html += '<div class="toast" id="toast"></div>\n'
    
    # کارت اصلی
    html += '<div class="card" id="mainCard">\n'
    
    # هدر
    html += '<div class="card-header">\n'
    html += '<div class="brand"><div class="brand-icon">🪐</div><div><div class="brand-text">پنل عقاب</div><div class="brand-sub">اشتراک ویژه</div></div></div>\n'
    html += '<button class="theme-toggle" onclick="toggleTheme()" id="themeBtn">🌙</button>\n'
    html += '</div>\n'
    
    # نام کاربر
    html += '<div class="user-row">\n'
    html += '<div class="user-name"><span>🪐</span> ' + label + '</div>\n'
    html += '<span class="status-badge ' + ('active' if is_allowed else 'inactive') + '"><i class="ti ' + ('ti-circle-check' if is_allowed else 'ti-circle-x') + '"></i>' + ('فعال' if is_allowed else 'غیرفعال') + '</span>\n'
    html += '</div>\n'
    html += '<div class="uuid-box" onclick="copyUUID()">🔑 ' + uuid + '</div>\n'
    
    # اتصالات
    if active_connections > 0:
        html += '<div class="conns-section"><div class="conns-header"><span class="conns-dot"></span> ' + str(active_connections) + ' دستگاه متصل</div><div class="conns-list">'
        for conn in active_connections_list[:10]:
            ip = conn.get('ip', 'نامشخص')
            html += '<span class="conn-chip">🔵 ' + ip + '</span>'
        if len(active_connections_list) > 10:
            html += '<span class="conn-chip more">+' + str(len(active_connections_list)-10) + '</span>'
        html += '</div></div>'
    else:
        html += '<div class="conns-section"><div class="conns-header">🔴 بدون اتصال فعال</div></div>'
    
    # اطلاعات
    html += '<div class="info-grid">\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-database"></i> مصرف</span><span class="info-value used">' + used_fmt + '</span></div>\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-package"></i> سهمیه</span><span class="info-value">' + limit_fmt + '</span></div>\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-devices"></i> دستگاه</span><span class="info-value">' + (str(max_devices) if max_devices > 0 else '∞') + '</span></div>\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-ip"></i> IP</span><span class="info-value small">' + user_ip + '</span></div>\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-fingerprint"></i> FP</span><span class="info-value small">' + fp_emoji + ' ' + fp_name + '</span></div>\n'
    html += '<div class="info-item"><span class="info-label"><i class="ti ti-settings"></i> پروتکل</span><span class="info-value small">' + protocol + '</span></div>\n'
    html += '</div>\n'
    
    # تایمر
    html += '<div class="timer-section"><div class="timer-label">⏱ زمان باقیمانده</div><div class="timer-display" id="timerDisplay">' + days_left + '</div></div>\n'
    
    # نوار پیشرفت
    html += '<div class="progress-section"><div class="progress-bar"><div class="progress-fill" id="progressFill" style="width:' + str(percent) + '%"></div></div><div class="progress-text"><span>میزان مصرف</span><span class="pct">' + str(percent) + '%</span></div></div>\n'
    
    # لینک VLESS
    html += '<div class="vless-section"><div class="vless-label"><i class="ti ti-link"></i> لینک کانفیگ</div><div class="vless-link" id="vlessLink">' + vless_link + '</div><div class="vless-ports">'
    for i, p in enumerate(ports[:5]):
        html += '<span class="vless-port">🔌 ' + str(p) + '</span>'
    if len(ports) > 5:
        html += '<span class="vless-port">+' + str(len(ports)-5) + '</span>'
    html += '</div></div>\n'
    
    # دکمه‌ها
    html += '<div class="actions"><button class="btn btn-primary" onclick="copyVless()"><i class="ti ti-copy"></i> کپی</button><button class="btn btn-success" onclick="copySub()"><i class="ti ti-link"></i> ساب</button><button class="btn btn-secondary" onclick="showQR()"><i class="ti ti-qrcode"></i> QR</button></div>\n'
    
    html += '<div class="footer"><span class="eagle">🪐</span> پنل عقاب · نسخه ۱۰</div>\n'
    html += '</div>\n'
    
    # اسکریپت
    html += '<script>\n'
    html += 'const vless = `' + vless_link + '`;\n'
    html += 'const subUrl = `' + sub_url + '`;\n'
    html += 'const uuid = `' + uuid + '`;\n'
    html += 'const remainingSeconds = ' + str(remaining_seconds) + ';\n'
    html += 'const isExpired = ' + str(not is_allowed).lower() + ';\n'
    html += 'function toast(msg,type){const t=document.getElementById("toast");t.textContent=msg;t.className="toast show"+(type?" "+type:"");clearTimeout(t._timeout);t._timeout=setTimeout(()=>t.classList.remove("show"),2000)}\n'
    html += 'function copyVless(){navigator.clipboard.writeText(vless).then(()=>toast("✅ کپی شد","ok"))}\n'
    html += 'function copySub(){navigator.clipboard.writeText(subUrl).then(()=>toast("✅ کپی شد","ok"))}\n'
    html += 'function copyUUID(){navigator.clipboard.writeText(uuid).then(()=>toast("✅ کپی شد","ok"))}\n'
    html += 'function showQR(){window.open("https://api.qrserver.com/v1/create-qr-code/?size=250x250&data="+encodeURIComponent(vless),"_blank")}\n'
    html += 'let currentTheme=localStorage.getItem("eagle-sub-theme")||"dark";\n'
    html += 'function toggleTheme(){setTheme(currentTheme==="dark"?"light":"dark")}\n'
    html += 'function setTheme(theme){currentTheme=theme;localStorage.setItem("eagle-sub-theme",theme);document.documentElement.setAttribute("data-theme",theme);document.getElementById("themeBtn").textContent=theme==="dark"?"☀️":"🌙"}\n'
    html += 'setTheme(currentTheme);\n'
    html += 'function formatTime(seconds){if(seconds<=0)return"⏰ منقضی شد";const days=Math.floor(seconds/86400);const hours=Math.floor((seconds%86400)/3600);const mins=Math.floor((seconds%3600)/60);const secs=seconds%60;if(days>0){return days+"d "+String(hours).padStart(2,"0")+"h "+String(mins).padStart(2,"0")+"m"}return String(hours).padStart(2,"0")+":"+String(mins).padStart(2,"0")+":"+String(secs).padStart(2,"0")}\n'
    html += 'function updateTimer(){const el=document.getElementById("timerDisplay");if(isExpired){el.textContent="⏰ منقضی شد";el.className="timer-display expired";return}if(remainingSeconds<=0){el.textContent="♾️ نامحدود";el.className="timer-display";return}const elapsed=Math.floor(Date.now()/1000)-startTime;const remaining=Math.max(0,remainingSeconds-elapsed);el.textContent=formatTime(remaining);if(remaining===0){el.className="timer-display expired";el.textContent="⏰ منقضی شد"}}\n'
    html += 'const startTime=Math.floor(Date.now()/1000);\n'
    html += 'if(remainingSeconds>0&&!isExpired){updateTimer();setInterval(updateTimer,1000)}else if(isExpired){document.getElementById("timerDisplay").textContent="⏰ منقضی شد";document.getElementById("timerDisplay").className="timer-display expired"}\n'
    html += '</script>\n'
    html += '</body>\n'
    html += '</html>'
