...
<section class="pg" id="pg-users">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-users"></i> کاربران</div><div class="tb-sub">لیست کانفیگ‌ها، سهمیه و انقضا</div></div>
  ...
  <div style="background:var(--card);border:1px solid var(--card-b);border-radius:var(--radius);overflow:hidden;backdrop-filter:blur(20px);transition:background .4s">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-top:1px solid var(--card-b);flex-wrap:wrap;gap:8px;">
      ...
      <div style="display:flex;gap:6px;"><button class="btn btn-p btn-sm" onclick="openModal('modal-user')"><i class="ti ti-plus"></i> افزودن کاربر جدید</button></div>
    </div>
  </div>
</section>
...
async function loadThemeFromServer() {
    try { const r = await fetch('/api/settings'); const data = await r.json(); if (data.theme) { currentTheme = data.theme; localStorage.setItem('eagle-theme', data.theme); setTheme(data.theme); } else { setTheme(currentTheme); } } catch(e) { setTheme(currentTheme); }
}
...
async function updateInbound() {
    ...
    try {
        const r = await authF('/api/inbound', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ port: port }) });
...
async function loadUsers() {
    try {
        ...
        tbody.innerHTML = links.map(l => {
            ...
            let duration = '∞';
            if (l.expires_at) {
                try { const exp = new Date(l.expires_at); const now = new Date(); const days = Math.ceil((exp - now) / (1000*60*60*24)); duration = days > 0 ? days + ' روز' : 'منقضی'; } catch(e) { duration = '—'; }
            }
            
            const avatarLetter = (l.label || 'U')[0].toUpperCase();
            return `<tr>
                <td><div class="user-name-cell"><div class="avatar">${avatarLetter}</div><div><div class="name">${esc(l.label)}</div><div class="uuid-short">${l.uuid.slice(0,8)}…</div></div></div></td>
                ...
                <td><div class="usage-bar"><span class="usage-text">${usedFmt} / ${limitFmt}</span><div class="bar"><div class="fill" style="width:${pct}%"></div></div></div></td>
...
async function saveUser() {
    ...
    try {
        ...
        toast('✅ کاربر با ' + ports.length + ' پورت ساخته شد', 'ok');
...
async function confirmDelete() {
    ...
    try {
        const r = await authF('/api/links/' + uuid, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) });
        if (!r.ok) { if (r.status === 403) { toast('❌ رمز اشتباه', 'err'); return; } throw new Error(); }
...
async function loadRGBStatus() {
    try { const r = await authF('/api/settings'); const data = await r.json(); rgbMode = data.rgb_mode || false; updateRGBUI(); } catch(e) {}
}
...
async function toggleRGB() {
    ...
    try { const r = await authF('/api/settings/rgb', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled: newState }) }); const data = await r.json(); rgbMode = data.rgb_mode; updateRGBUI(); toast(rgbMode ? '🌈 RGB فعال شد' : '🌙 RGB غیرفعال شد', 'ok'); } catch(e) { toast('❌ خطا', 'err'); }
}
...
async function changePassword() {
    ...
    try {
        const r = await authF('/api/change-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ old_password: oldPw, new_password: newPw }) });
        const data = await r.json();
...
async function restoreBackup(event) {
    ...
    try {
        ...
        const r = await authF('/api/backup/restore', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
        if (!r.ok) { toast('❌ خطا', 'err'); return; }
...
# ===== تابع صفحه ساب‌لینک حرفه‌ای با تایمر و تغییر تم =====
def get_sub_page_html(uuid: str, link: dict) -> str:
    """صفحه ساب‌لینک با طراحی حرفه‌ای، تایمر زنده و تغییر تم"""
    ...
    # محاسبه زمان باقیمانده به ثانیه برای تایمر
    remaining_seconds = 0
    ...
    if active_connections > 0:
        ...
        <div class="conns-section">
            <div class="conns-header"><span class="conns-dot"></span> {active_connections} دستگاه متصل</div>
            <div class="conns-list">"""
        for conn in active_connections_list[:10]:
            ip = conn.get('ip', 'نامشخص')
            conns_html += f"""<span class="conn-chip">🔵 {ip}</span>"""
        if len(active_connections_list) > 10:
            conns_html += f"""<span class="conn-chip more">+{len(active_connections_list)-10}</span>"""
        conns_html += "</div></div>"
    else:
        conns_html = """<div class="conns-section offline"><div class="conns-header">🔴 بدون اتصال فعال</div></div>"""
...
:root {{
  ...
  --card-border: rgba(100, 80, 255, 0.06);
  --card-hover: rgba(100, 80, 255, 0.1);
  --text: #F0EEFF;
  --text2: #8888BB;
  --text3: #555577;
  --accent: #7C6BFF;
  ...
  --glow: 0 0 80px rgba(100, 80, 255, 0.03);
...
[data-theme="light"] {{
  ...
  --card-border: rgba(100, 80, 255, 0.12);
  --card-hover: rgba(100, 80, 255, 0.15);
  --text: #1a1a2e;
  --text2: #4a4a6a;
  --text3: #7a7a9a;
  --shadow: 0 25px 80px rgba(0, 0, 0, 0.08), 0 0 120px rgba(100, 80, 255, 0.02);
  --glow: 0 0 80px rgba(100, 80, 255, 0.02);
}}
...
.brand-icon {{
  ...
  justify-content: center;
  font-size: 16px;
  box-shadow: 0 0 30px rgba(100, 80, 255, 0.15);
}}
.brand-text {{
  font-size: 11px;
  font-weight: 700;
...
.brand-sub {{ font-size: 6px; color: var(--text3); -webkit-text-fill-color: var(--text3); }}
.theme-toggle {{
...
.conns-section.offline {{ border-color: rgba(239, 68, 68, 0.06); }}
.conns-header {{
  font-size: 9px;
  font-weight: 700;
  color: var(--text2);
...
.conns-list {{ display: flex; flex-wrap: wrap; gap: 3px; }}
.conn-chip {{
  font-family: monospace;
  font-size: 7px;
...
.info-value.green {{ color: var(--green-text); }}
.info-value.used {{ color: var(--accent2); }}

/* ===== تایمر ===== */
.timer-section {{
  background: linear-gradient(135deg, rgba(100, 80, 255, 0.04), rgba(167, 139, 250, 0.02));
  border: 1px solid var(--card-border);
  border-radius: 8px;
  padding: 6px 10px;
  margin: 6px 0;
...
.btn-primary {{
  background: linear-gradient(135deg, #7C6BFF, #5B4BD9);
  color: #fff;
  box-shadow: 0 3px 15px rgba(100, 80, 255, 0.15);
}}
.btn-primary:hover {{
  transform: translateY(-2px);
  box-shadow: 0 6px 25px rgba(100, 80, 255, 0.25);
}}
.btn-secondary {{
  background: rgba(100, 80, 255, 0.03);
  border: 1px solid var(--card-border);
...
<div class="card" id="mainCard">
    ...
    <div class="info-grid">
        ...
        <div class="info-item"><span class="info-label"><i class="ti ti-ip"></i> IP</span><span class="info-value small">{user_ip}</span></div>
        <div class="info-item"><span class="info-label"><i class="ti ti-fingerprint"></i> FP</span><span class="info-value small">{fp_emoji} {fp_name}</span></div>
    ...
    <div class="vless-section">
        ...
        <div class="vless-ports">
            {''.join([f'<span class="vless-port">🔌 {p}</span>' for p in ports[:5]])}
            {f'<span class="vless-port">+{len(ports)-5}</span>' if len(ports) > 5 else ''}
        </div>
    </div>

    <!-- دکمه‌ها -->
    <div class="actions">
        <button class="btn btn-primary" onclick="copyVless()"><i class="ti ti-copy"></i> کپی</button>
        <button class="btn btn-success" onclick="copySub()"><i class="ti ti-link"></i> ساب</button>
        <button class="btn btn-secondary" onclick="showQR()"><i class="ti ti-qrcode"></i> QR</button>
    </div>

    <div class="footer"><span class="eagle">🪐</span> پنل عقاب · نسخه ۱۰</div>
</div>
...
// ===== مدیریت تم =====
let currentTheme = localStorage.getItem('eagle-sub-theme') || 'dark';
